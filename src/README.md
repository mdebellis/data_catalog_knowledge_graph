...
2. For each property in the property file it finds all matching triples with that property as the predicate and generates text strings that describe that triple in a way the LLM can understand. So using dp:has_bounded_context as an example, it finds all triples that match:

```?subject dp:has_bounded_context ?object``` It uses the rdfs:label for the ?subject, the dp:embedding_label for the property if one exists and otherwise uses the property's rdfs:label. Finally, if the ?object is an IRI in the graph it finds the rds:label. If the ?object is a literal it turns it into a string (without the type info). This is especially useful
